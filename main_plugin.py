...
class EnhancedAI:
    ...
    def generate_emotional_response(self, sentiment: int, context: List[str] = None) -> Optional[str]:
        ...
        if sentiment > 1:
            ...
            if random.random() < 0.5:
                response = random.choice(neutral_responses) + random.choice(self.emoji_words)
            else:
    ...
    async def find_most_relevant(self, query: str, context: List[str] = None, topic: List[str] = None) -> Optional[str]:
        ...
        # 如果训练后vectorizer仍然不存在或未训练，返回None
        if not self.vectorizer or not hasattr(self.vectorizer, 'vocabulary_'):
            logger.warning("向量化器未初始化或词汇表未拟合，无法查找相关回复")
            return None
    ...
    def _add_logical_connectors(self, response: str, context: List[str]) -> str:
        """为回复添加逻辑连接词 - 提高自然度和适应性"""
        # 提高连接词添加概率到60%
        if not context or random.random() > 0.6:
            return response
        ...
        # 根据逻辑关系选择合适的连接词
        if connector_type and connector_type in self.logical_connectors:
            connector = random.choice(self.logical_connectors[connector_type])
            return f"{connector}，{response}"
        
        # 如果没有检测到逻辑关系，随机选择连接词类型
        if random.random() < 0.5:
            # 优先选择补充或回应类型
            connector_type = random.choice(["补充", "回应"])
            connector = random.choice(self.logical_connectors[connector_type])
            return f"{connector}，{response}"
    ...
    def _improve_response_flow(self, response: str, conversation_state: Dict) -> str:
        ...
        if depth > 1:
            ...
            # 问题模式下，提供更直接的回答
            if len(response) > 20 and random.random() < 0.4:
                # 太长的回答可以简化
        ...
        if random.random() < 0.3:
            response += " " + random.choice(self.emoji_words)
    ...
    async def generate_response(self, group_id: int, user_id: int, message: str) -> str:
        ...
        if config.emotional_response_enabled:
            ...
            if group_ambience['positive'] > 0.6:
                emotional_prob = 0.4  # 积极氛围下提高情感回应概率
            elif group_ambience['negative'] > 0.5:
                emotional_prob = 0.5  # 消极氛围下提高情感回应概率
            else:
                emotional_prob = 0.3  # 中性氛围
            
            if random.random() < emotional_prob:
        ...
        # 5. 尝试找到最相关的回复 (优先使用增强学习系统)
        if config.enable_semantic_analysis:
            # 首先尝试使用增强学习系统
            relevant_reply = self.enhanced_training.find_relevant_reply(message)
        ...
        # 7. 根据群聊氛围生成参与性回复
        if random.random() < 0.3:
            participation_reply = self._generate_participation_reply(group_ambience, hot_topics)
        ...
        if group_ambience['positive'] > 0.6:
            positive_defaults = [
                "大家聊得真开心呢~",
                "今天的气氛真好呀！",
                "哈哈，大家说得都很有道理" + random.choice(self.emoji_words)
            ]
            return random.choice(positive_defaults)
        elif group_ambience['negative'] > 0.5:
            negative_defaults = [
                "是不是有点不开心呀？",
                "大家要不要换个开心的话题？",
            ...
            return random.choice(negative_defaults)
        else:
            return random.choice(default_responses)
        
    def _generate_participation_reply(self, group_ambience: Dict[str, float], hot_topics: List[str]) -> Optional[str]:
        """生成参与性回复，让机器人更好地融入群聊"""
        # 根据群氛围生成参与性回复
        if group_ambience['positive'] > 0.6:
